...
class NewsSourceService(CommonService):
    ...
    def get_by_tenant_id(
        cls, tenant_id: str, page: int = 1, page_size: int = 20, name: Optional[str] = None, status: Optional[str] = None, source_type: Optional[str] = None, source_types: Optional[List[str]] = None
    ):
        """根据租户ID获取新闻源列表"""
    ...
    def update_source(cls, source_id: str, tenant_id: str, **kwargs):
        ...
        if update_data:
            cls.model.update(**update_data).where(cls.model.id == source_id).execute()
    ...
    def get_by_types(cls, tenant_id: str, source_types: List[str]) -> List[dict]:
        if not source_types:
            return []
        query = cls.model.select().where((cls.model.tenant_id == tenant_id) & (cls.model.status != "deleted") & (cls.model.source_type.in_(source_types)))
...
class NewsTaskService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def get_by_tenant_id(cls, tenant_id: str, page: int = 1, page_size: int = 20, task_name: Optional[str] = None, status: Optional[str] = None):
        """根据租户ID获取任务列表"""
    ...
    def update_task(cls, task_id: str, tenant_id: str, **kwargs):
        ...
        update_data = {}
        for field in ["task_name", "source_ids", "auto_parse", "max_articles_per_source", "crawler_config"]:
            if field in kwargs:
                update_data[field] = kwargs[field]

        if update_data:
            cls.model.update(**update_data).where(cls.model.id == task_id).execute()
    ...
    def get_pending_tasks(cls, limit: int = 10):
        """获取待执行的任务"""
        tasks = cls.model.select().where(cls.model.status == "pending").limit(limit)
    ...
    def to_dict(cls, obj):
        ...
        if hasattr(obj, "statistics") and obj.statistics:
            ...
            result["statistics"] = {"total_articles": 0, "success_count": 0, "failed_count": 0, "skipped_count": 0}
...
class NewsContentService(CommonService):
    ...
    def get_statistics_by_time_range(cls, tenant_id: str, start_time: int, end_time: int):
        """获取时间范围内的统计数据"""
        query = cls.model.select().where((cls.model.tenant_id == tenant_id) & (cls.model.fetch_time >= start_time) & (cls.model.fetch_time <= end_time))
        ...
        return {"total_articles": total_count, "source_distribution": source_stats, "time_range": {"start": start_time, "end": end_time}}
    ...
    def get_all_content_hashes(cls, tenant_id: str) -> set:
        """高效获取指定租户的所有内容哈希值"""
        query = cls.model.select(cls.model.content_hash).where((cls.model.tenant_id == tenant_id) & (cls.model.content_hash.is_null(False)))
    ...
    def get_hashes_paginated(cls, tenant_id: str, page: int = 1, page_size: int = 20):
        """(最终修正版) 分页获取内容的简略信息"""
        query = cls.model.select().where(cls.model.tenant_id == tenant_id).order_by(cls.model.create_time.desc())
        ...
        for record in records:
            ...
            result.append({"id": getattr(record, "id", None), "title": getattr(record, "title", "No Title"), "content_hash": getattr(record, "content_hash", None), "create_time": create_time_iso})
