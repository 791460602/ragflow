...
def list_news_tasks():
    ...
    try:
        ...
        tasks, total = NewsTaskService.get_by_tenant_id(tenant_id=current_user.id, page=page, page_size=page_size, task_name=task_name, status=status)
...
def create_news_task():
    ...
    try:
        ...
        task = NewsTaskService.create_task(tenant_id=current_user.id, user_id=current_user.id, **req)
...
def update_news_task(task_id):
    ...
    try:
        ...
        task = NewsTaskService.update_task(task_id=task_id, tenant_id=current_user.id, **req)
...
def get_news_statistics():
    ...
    try:
        ...
        # 获取时间范围内的内容统计
        content_stats = NewsContentService.get_statistics_by_time_range(current_user.id, start_time, end_time)
...
def ping():
    """服务状态检查"""
    try:
        return get_json_result(data={"status": "ok", "timestamp": datetime.now().isoformat(), "user_id": current_user.id, "version": "1.0.0"})
...
def get_available_crawlers():
    """获取可用爬虫列表"""
    try:
        crawlers = [{"type": "demo", "name": "Demo爬虫", "description": "演示爬虫 - 生成示例新闻数据"}]
