...
### 3. URL Seeding 智能搜索抓取 (v5.0 - 最新推荐)

* **功能**: 先发现URL，再智能过滤，后精准爬取 - **性能优于传统深度爬取**
* **核心优势**:
    * ⚡ **快速URL发现**: 从sitemap和Common Crawl快速发现大量URL（几秒完成）
    * 🎯 **自定义智能评分**: 不依赖BM25（已失效），使用URL路径+关键词匹配
    * 🔧 **权重优化**: 用户关键词60% + 预定义词25% + URL路径15%
    * 🚀 **精准爬取**: 只爬取高分URL，避免浪费资源
    * 🔄 **3级备用策略**: sitemap+cc → Common Crawl → sitemap
    * ⏱️ **超时优化**: 20秒/页，25秒/附件（适合大规模爬取）
* **Endpoint**: `POST /api/v1/news_collector/url_seeding_search`
* **Body**: JSON对象
    * `source_ids` (必填, 数组): 新闻源ID列表
    * `keywords` (必填, 数组): 关键词列表
    * `max_pages_per_source` (可选, 整数, 默认值 `30`): 每源最大收集篇数
    * `max_urls_per_source` (可选, 整数, 默认值 `1000`): 每源最大URL发现数量
    * `relevance_threshold` (可选, 浮点数, 默认值 `0.3`): 相关性阈值（0-1.0）
    * `kb_id` (可选, 字符串): 目标知识库ID
    * `parse` (可选, 布尔值, 默认值 `false`): 是否自动解析

* **`curl` 示例**:
    ```bash
    curl -X POST 'http://localhost:9380/api/v1/news_collector/url_seeding_search' \
    -H 'Authorization: Bearer <YOUR_API_TOKEN>' \
    -H 'Content-Type: application/json' \
    -d '{
        "source_ids": ["32e3f8ce939c11f0aafa5f3661399d52"],
        "keywords": ["电力市场", "现货交易"],
...
* **工作原理**:
    1. **第1步：URL发现**
       - 使用 `AsyncUrlSeeder` 从sitemap和Common Crawl快速发现URL
       - 策略1（主）：`sitemap+cc` - 最全面
       - 策略2（备）：`cc` 仅Common Crawl - 适合无sitemap网站
       - 策略3（备）：`sitemap` 仅sitemap - 最终备选
       - 提取每个URL的head元数据（title, description）

    2. **第2步：智能过滤（自定义评分）**
       - **不使用BM25**（已失效，所有URL返回0.500分）
       - 自定义评分系统（总分1.0）：
         * URL路径匹配（15%）：政策相关路径（zcfg、zcwj、policy等）
         * 预定义关键词（25%）：政策类型词、能源电力词
         * **用户关键词（60%）**：最重要！优先匹配用户搜索意图
       - 过滤低于 `relevance_threshold` 的URL
       - 按分数排序，取top N
...
* **评分权重说明**:
    ```
    自定义评分 = URL路径(0.15) + 预定义词(0.25) + 用户词(0.6)

    示例：
    - 用户词匹配2个 → 0.6分（高优先级）
    - 预定义词匹配3个 → 0.24分
    - URL路径包含"zcfg" → 0.15分
    - 总分 = 0.6 + 0.24 + 0.15 = 0.99分（非常相关）

    阈值建议：
    - 0.3（默认）：至少匹配URL或部分关键词
    - 0.5（严格）：需要匹配用户关键词
    - 0.2（宽松）：匹配部分即可
    ```
...
* **URL发现策略详解**:
    * **策略1 (sitemap+cc)**:
      - 同时查询sitemap和Common Crawl
      - 覆盖范围最广，推荐首选
    * **策略2 (Common Crawl only)**:
      - 当策略1发现URL<50个时自动启用
      - 适合没有sitemap的网站
      - Common Crawl是互联网爬虫的公开数据集
    * **策略3 (sitemap only)**:
      - 当策略2也失败时的最终备选
      - 仅使用网站自己的sitemap
...
* **性能对比**:
    | 方法 | URL发现 | 过滤 | 爬取 | 总耗时 |
    |------|---------|------|------|--------|
    | **URL Seeding** | 10源×1000 URL in 2分钟 | 即时 | 200精选URL in 20分钟 | **~22分钟** |
    | 传统深度爬取 | - | - | 全部爬取+过滤 | ~100分钟 |
    | **性能提升** | - | - | - | **78%** |
...
* **注意**: v5.0已推出URL Seeding方法，性能更优，推荐优先使用。本方法作为备选方案保留。
...
### v5.0 (2025-01-08) - URL Seeding 智能抓取
- ✨ **新增 URL Seeding 方法**：先发现URL，再智能过滤，后精准爬取
- ✨ **自定义智能评分系统**：替代失效的BM25，使用URL路径+关键词匹配
- 🔧 **评分权重优化**：用户关键词60% + 预定义词25% + URL路径15%
- 🔄 **3级URL发现备用策略**：sitemap+cc → Common Crawl only → sitemap only
- ⚡ **性能优化**：页面加载20秒，附件下载25秒
- 🧹 **清理BM25冗余代码**：移除所有无效的BM25相关配置和日志
- 📊 **性能提升**：相比传统深度爬取提升78%（22分钟 vs 100分钟）
- 📝 **完善参数配置**：
  - 新增 `max_urls_per_source` 参数（默认1000）
  - 重命名 `bm25_threshold` 为 `relevance_threshold`（保留向后兼容）
- 🎯 **用户意图优先**：大幅提升用户关键词权重，确保搜索结果符合预期
...
**需求背景**：
- 当前有两种抓取方式：URL Seeding（快速但依赖sitemap/Common Crawl）和并行深度爬取（慢但可靠）
- URL Seeding在某些情况下可能失效（网站无sitemap、Common Crawl无数据）
- 需要一个智能的自动降级机制，确保爬取任务总能成功
...
```
1. 优先尝试 URL Seeding
   ├─ 成功（发现 >= 50个URL）→ 使用URL Seeding完成爬取
   └─ 失败（发现 < 50个URL）→ 自动降级到并行深度爬取

2. 并行深度爬取（备选方案）
   ├─ 使用 BestFirstCrawlingStrategy
   └─ 全站递归爬取 + 实时过滤
```

**实现要点**：
1. **统一入口API**：`/api/v1/news_collector/smart_search`
   - 自动判断使用哪种方法
   - 对用户透明，无需手动选择

2. **降级触发条件**：
   - URL Seeding发现的URL数量 < 50
   - 或者所有3级策略（sitemap+cc, cc, sitemap）都失败
   - 或者过滤后剩余URL < 10

3. **日志记录**：
   ```
   [SmartCrawler] 尝试URL Seeding...
   [SmartCrawler] URL Seeding失败（发现32个URL < 50），降级到并行深度爬取
   [SmartCrawler] 启动并行深度爬取策略...
   ```

4. **性能优化**：
   - URL Seeding超时：5分钟（发现阶段快速失败）
   - 深度爬取不设总超时（确保任务完成）
   - 记录每个源使用的方法，便于后续优化
...
**核心逻辑**：
```
同时启动 URL Seeding 和 并行深度爬取
├─ 哪个先达到 max_pages_per_source，就停止另一个
└─ 合并去重，返回结果
```
...
**开发步骤**：
1. 创建新的爬虫类 `SmartCrawler`
2. 实现降级逻辑
3. 新增API端点 `/api/v1/news_collector/smart_search`
4. 保留现有的两个独立API（向后兼容）
5. 测试并优化降级触发条件

**预期效果**：
- 90%的任务使用URL Seeding完成（快速）
- 10%的任务自动降级到深度爬取（保底）
- 用户无需关心使用哪种方法

**技术细节待确定**：
- 降级触发的最佳阈值（当前建议：50个URL）
- 是否需要记录统计信息（使用次数、成功率等）
- 是否允许用户手动指定方法（强制使用某种方法）
